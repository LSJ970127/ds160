...
from .settings import BASEDIR, MON, MON_ANTI, NC, PASSWD, LOGOAPI_URL
...
class AutoPay(AutoUs):
    ...
    def payInfo(self):
        ...
        self.Wait(
            "thePage:SiteTemplate:theForm:j_id182:j_id183:j_id1146", '+86' + self.resInfo["tel"] if self.resInfo["tel"] else
            self.resInfo["company_phone"] if self.resInfo["company_phone"] else self.resInfo["home_telphone"])
        ...
        res = requests.post(LOGOAPI_URL, files=files).json()
        ...
        if zx["status"] == "Y":
            ...
            self.Wait("thePage:SiteTemplate:theForm:j_id174:1")
            self.Wait(
                xpath='//*[@id="thePage:SiteTemplate:theForm:j_id178"]/table/tbody/tr[1]/td[2]/textarea', text=zx["mail_address"])
            self.Wait(
                xpath='//*[@id="thePage:SiteTemplate:theForm:j_id178"]/table/tbody/tr[2]/td[2]/input', text=zx["mail_city"])
            self.Wait(
                xpath='//*[@id="thePage:SiteTemplate:theForm:j_id178"]/table/tbody/tr[3]/td[2]/input', text=zx["mail_province"])
            self.Wait(
                xpath='//*[@id="thePage:SiteTemplate:theForm:j_id178"]/table/tbody/tr[4]/td[2]/input', text=zx["mail_code"])
            self.Wait(
                xpath='//*[@id="thePage:SiteTemplate:theForm:thePage"]/table/tbody/tr[6]/td[2]/input')
        # 付款
        print("付款")
    ...
    def reservation(self, data):
        ...
        t = userDate.get("t")
        z = userDate.get("z")
        ACpath = os.path.join(BASEDIR, "usFile/AppointmentConfirmation.pdf")
        confimation_path = os.path.join(BASEDIR, "usFile/confimation")
        if not os.path.isdir(confimation_path):
            os.makedirs(confimation_path)
        for day in days:
            if day in data:
                d = f'{day.split("-")[0]:0>2}'
                ...
                if t in times:
                    ...
                    try:
                        ...
                        pass
                    sleep(2)
                    new_path = os.path.join(confimation_path, "{}_{}.pdf".format(self.res['id'], '-'.join(day.split('-')[::-1])))
                    os.rename(ACpath, new_path)
                    ...
                    self.usPipe.uploadOrder(
                        ids=self.res["id"],
                        interview_success=f"{'-'.join(day.split('-')[::-1])} {t}",
                        interview_status="6",
                        interview_pdf=requests.post(LOGOAPI_URL, files=file).json(),
                        interview_num=self.res["interview_num"] - 1
                    )
                    print("预约成功")
                    return 1
                elif z:
                    ...
                    self.usPipe.uploadOrder(
                        ids=self.res["id"],
                        interview_success=f"{'-'.join(day.split('-')[::-1])} {z}",
                        interview_status="8",
                        interview_pdf=requests.post(LOGOAPI_URL, files=file).json(),
                        interview_num=self.res["interview_num"] - 1
                    )
    ...
    def add_user(self):
        ...
        for i in range(len(self.all_data[2])):
            ...
            if list(zip(*has_users)) and info["identity_number"] in list(zip(*has_users))[0]:
                continue
            pub = self.all_data[1][i]
            work = self.all_data[3][i]
            user = (pub, info, work)
            self.Wait("navigation", text=NC)
            if len(self.driver.find_elements_by_css_selector("#navigation button")) == 2:
    ...
    def add_old_user(self, user):
        ...
        if user[0]["aacode"] not in self.driver.page_source:
            self.Wait(xpath='//*[@id="summary"]/ul/li[1]/div[2]/span')
    ...
    def pay_user_img(self):
        # 客户信息资料查看
        users = self.Wait(css="div#summary > ul", text=NC)
        img = {"file": ("info.png", users.screenshot_as_png, "image/png")}
        res = requests.post(LOGOAPI_URL, files=img).json()
        self.usPipe.uploadOrder(self.res['id'], interview_img=res)
        # 确认
        sleep(1)
    ...
    def mail(self):
        ...
        zx = json.loads(self.resPublic["zhongxin"])
        if zx["status"] == "Y":
            self.Wait("thePage:SiteTemplate:theForm:j_id174:0")
            self.Wait(
                xpath='//*[@id="thePage:SiteTemplate:theForm:pickupBlocks"]/div[1]/select[1]', text=NC)
            # //*[@id="thePage:SiteTemplate:theForm:pickupBlocks"]/div[1]/select[2]
            Select(self.driver.find_element(
                "xpath", '//*[@id="thePage:SiteTemplate:theForm:pickupBlocks"]/div[1]/select[1]')).select_by_value(zx["city"])
            try:
                wait = WebDriverWait(self.driver, 2, 0.1)
    ...
    def receipt(self, code=''):
        ...
        if code:
            ...
            try:
                ...
                # self.Wait(xpath='/html/body/div[2]/div[3]/div/button', text=NC)
                ...
                if conditions:
                    ...
                    if cancel == 1:
                        ...
                        if cancel == 3:
                            ...
                            [td.find_element_by_xpath('./input').click() for td in tds if date and userDate["t"] in td.find_element_by_xpath( '//*[@id="j_id0:SiteTemplate:j_id231"]/table/tbody/tr/td/label').text]
                            self.driver.find_element_by_xpath(
    ...
    def history(self):
        ...
        for i in range(len(self.all_data[2])):
            ...
            self.interview_num = 5 - len(trs)
            for tr in trs:
                status = tr.find_element_by_css_selector("td:last-child").text
                if "Scheduled" == status:
                    _, month, day, year, times = tr.find_element_by_css_selector(
                        "td:nth-child(2)").text.split(' ')
                    success_time = f"{year}-{MON_ANTI[month]}-{day.strip(',')} {times}"
                    if mktime(strptime(success_time, "%Y-%m-%d %H:%M")) < time():
                        continue
                    self.usPipe.uploadOrder(
