...
class UsPipeline(Mysql):
    ...
    def upload(self, aid=None, **kwargs):
        ...
        try:
            ...
            print('未传 aid 或者 aaCode/status 无值')
    ...
    def selDBOrder(self, usql=None):
        ...
        # 测试
        # sql = "SELECT * FROM dc_business_america_order WHERE id=141"
        self.cur.execute(sql)
    ...
    def uploadOrder(self, ids=None, **kwargs):
        ...
        sql = f"UPDATE dc_business_america_order SET {cSql}, utime='{int(time())}' WHERE id='{ids}'"
        try:
            self.cur.execute(sql)
            self._conn.commit()
        except Exception as e:
